  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="tags">
        @foreach ($tags as $tag)
          <span data-name="{{ strtolower($tag->name) }}" class="badge badge-pill m-1 {{ (! empty($article) && in_array($tag->id, $article->resources()->tagsIds())) ? 'selected' : '' }}" data-id="{{ $tag->id }}">
            @if(empty($article))
            <input type="checkbox" data-name="{{ strtolower($tag->name) }}" style="visibility: hidden; position: absolute;" name="tags[]" value="{{$tag->id}}">
            @endif
